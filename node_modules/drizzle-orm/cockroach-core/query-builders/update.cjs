...
var CockroachUpdateBase = class extends __query_promise_ts.QueryPromise {
	...
	*/
	...
	_prepare(name, generateName = false) {
		...
		const preparedQuery = this.session.prepareQuery(query, this.config.returning, name ?? (generateName ? (0, __query_name_generator_ts.preparedStatementName)(query.sql, query.params) : name), true);
