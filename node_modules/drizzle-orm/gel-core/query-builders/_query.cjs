...
//#region src/gel-core/query-builders/_query.ts
var _RelationalQueryBuilder = class {
	static [__entity_ts.entityKind] = "GelRelationalQueryBuilder";
	...
	findMany(config) {
		return new GelRelationalQuery(this.fullSchema, this.schema, this.tableNamesMap, this.table, this.tableConfig, this.dialect, this.session, config ? config : {}, "many");
	}
	findFirst(config) {
		return new GelRelationalQuery(this.fullSchema, this.schema, this.tableNamesMap, this.table, this.tableConfig, this.dialect, this.session, config ? {
			...config,
			limit: 1
		} : { limit: 1 }, "first");
	}
};
var GelRelationalQuery = class extends __query_promise_ts.QueryPromise {
	static [__entity_ts.entityKind] = "GelRelationalQuery";
	...
	_prepare(name, generateName = false) {
		return __tracing_ts.tracer.startActiveSpan("drizzle.prepareQuery", () => {
			...
			return this.session.prepareQuery(builtQuery, void 0, name ?? (generateName ? (0, __query_name_generator_ts.preparedStatementName)(builtQuery.sql, builtQuery.params) : name), true, (rawRows, mapColumnValue) => {
...
//#endregion
exports.GelRelationalQuery = GelRelationalQuery;
