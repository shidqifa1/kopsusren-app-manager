...
//#region src/gel-core/query-builders/update.ts
var GelUpdateBuilder = class {
	static [__entity_ts.entityKind] = "GelUpdateBuilder";
	...
	set(values) {
		return new GelUpdateBase(this.table, (0, __utils_ts.mapUpdateSet)(this.table, values), this.session, this.dialect, this.withList);
	}
};
var GelUpdateBase = class extends __query_promise_ts.QueryPromise {
	static [__entity_ts.entityKind] = "GelUpdate";
	config;
	tableName;
	joinsNotNullableMap;
	...
	from(source) {
		const tableName = (0, __utils_ts.getTableLikeName)(source);
		if (typeof tableName === "string") this.joinsNotNullableMap[tableName] = true;
		this.config.from = source;
		return this;
	}
	getTableLikeFields(table) {
		if ((0, __entity_ts.is)(table, __gel_core_table_ts.GelTable)) return table[__table_ts.Table.Symbol.Columns];
	...
	*/
	...
	_prepare(name, generateName = false) {
		...
		const preparedQuery = this.session.prepareQuery(query, this.config.returning, name ?? (generateName ? (0, __query_name_generator_ts.preparedStatementName)(query.sql, query.params) : name), true, void 0, {
			type: "update",
...
//#endregion
exports.GelUpdateBase = GelUpdateBase;
exports.GelUpdateBuilder = GelUpdateBuilder;
