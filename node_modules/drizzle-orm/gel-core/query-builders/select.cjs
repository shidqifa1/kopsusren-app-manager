...
let __gel_core_view_base_ts = require("../view-base.cjs");

//#region src/gel-core/query-builders/select.ts
var GelSelectBuilder = class {
	static [__entity_ts.entityKind] = "GelSelectBuilder";
	fields;
	session;
	dialect;
	withList = [];
	...
	*/
	from(source) {
		...
		else if ((0, __entity_ts.is)(source, __gel_core_view_base_ts.GelViewBase)) fields = source[__view_common_ts.ViewBaseConfig].selectedFields;
...
var GelSelectQueryBuilderBase = class extends __query_builders_query_builder_ts.TypedQueryBuilder {
	static [__entity_ts.entityKind] = "GelSelectQueryBuilder";
	_;
	config;
	joinsNotNullableMap;
	tableName;
	...
	constructor({ table, fields, isPartialSelect, session, dialect, withList, distinct }) {
		...
		for (const item of require_gel_core_utils.extractUsedTable(table)) this.usedTables.add(item);
	}
	/** @internal */
	getUsedTables() {
	...
	*/
	...
	as(alias) {
		const usedTables = [];
		usedTables.push(...require_gel_core_utils.extractUsedTable(this.config.table));
		if (this.config.joins) for (const it of this.config.joins) usedTables.push(...require_gel_core_utils.extractUsedTable(it.table));
...
var GelSelectBase = class extends GelSelectQueryBuilderBase {
	static [__entity_ts.entityKind] = "GelSelect";
	/** @internal */
	_prepare(name, generateName = false) {
		...
		return __tracing_ts.tracer.startActiveSpan("drizzle.prepareQuery", () => {
			...
			const preparedQuery = session.prepareQuery(query, fieldsList, name ?? (generateName ? (0, __query_name_generator_ts.preparedStatementName)(query.sql, query.params) : name), true, void 0, {
				type: "select",
				tables: [...usedTables]
			}, cacheConfig);
...
(0, __utils_ts.applyMixins)(GelSelectBase, [__query_promise_ts.QueryPromise]);
...
//#endregion
exports.GelSelectBase = GelSelectBase;
exports.GelSelectBuilder = GelSelectBuilder;
exports.GelSelectQueryBuilderBase = GelSelectQueryBuilderBase;
