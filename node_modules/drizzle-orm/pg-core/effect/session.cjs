...
const migrate = effect_Effect.fn("migrate")(function* (migrations, session, config) {
	...
	const dbMigrations = yield* session.all(__sql_sql_ts.sql`select id, hash, created_at, name from ${__sql_sql_ts.sql.identifier(migrationsSchema)}.${__sql_sql_ts.sql.identifier(migrationsTable)}`);
