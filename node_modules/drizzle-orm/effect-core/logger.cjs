...
* ```
*/
var EffectLogger = class EffectLogger extends effect_Effect.Service()("drizzle-orm/EffectLogger", {
	sync: () => {
		const logQuery = (_query, _params) => effect_Effect.void;
		return { logQuery };
	},
	accessors: true
}) {
	static [__entity_ts.entityKind] = this.Service._tag;
	/**
	...
	*/
	static fromDrizzle(logger) {
		return new EffectLogger({ logQuery: (query, params) => {
	...
	* ```
	*/
	static layer = effect_Layer.succeed(EffectLogger, new EffectLogger({ logQuery: effect_Effect.fn("EffectLogger.logQuery")(function* (query, params) {
