...
function isOptional(schema) {
	...
	return !effect.Schema.isSchema(schema) && "from" in schema && effect.Schema.isSchema(schema.from);
}
...
function handleColumns(columns, refinements, conditions) {
	...
	for (const [key, selected] of Object.entries(columns)) {
		...
		const refined = isOptional(_refined) ? _refined.from : _refined;
...
function handleEnum(enum_) {
	return effect.Schema.Literal(...enum_.enumValues);
}
