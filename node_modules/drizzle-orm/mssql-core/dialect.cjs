			...
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) && field.isSelectionField) {
				...
				if (isSingleTable) {
					const newSql = new __sql_sql_ts.SQL(query.queryChunks.map((c) => {
						if ((0, __entity_ts.is)(c, require_mssql_core_columns_common.MsSqlColumn)) return __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(c));
						return c;
					}));
		...
		const chunks = fields.flatMap(({ field }, i) => {
			...
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) && field.isSelectionField) chunk.push(__sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), __sql_sql_ts.sql.identifier(field.fieldAlias)]));
				...
				chunk.push(new __sql_sql_ts.SQL(query.queryChunks.map((c) => {
					if ((0, __entity_ts.is)(c, require_mssql_core_columns_common.MsSqlColumn)) return __sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(c))]);
					return c;
				})));
				if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased)) chunk.push(__sql_sql_ts.sql` as ${__sql_sql_ts.sql.identifier(field.fieldAlias)}`);
			} else if ((0, __entity_ts.is)(field, __column_ts.Column)) chunk.push(__sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), field.isAlias ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier(this.casing.getColumnCasing((0, __alias_ts.getOriginalColumnFromAlias)(field)))} as ${field}` : __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(field))]));
			if (i < columnsLen - 1) chunk.push(__sql_sql_ts.sql`, `);
			return chunk;
		});
		return __sql_sql_ts.sql.join(chunks);
