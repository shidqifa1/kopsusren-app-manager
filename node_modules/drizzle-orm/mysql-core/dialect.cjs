	...
	*/
			...
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) && field.isSelectionField) {
				...
				if (isSingleTable) {
					const newSql = new __sql_sql_ts.SQL(query.queryChunks.map((c) => {
						if ((0, __entity_ts.is)(c, require_mysql_core_columns_common.MySqlColumn)) return __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(c));
						return c;
					}));
	...
	buildSetOperationQuery({ leftSelect, setOperator: { type, isAll, rightSelect, limit, orderBy, offset } }) {
		...
		if (orderBy && orderBy.length > 0) {
			const orderByValues = [];
			for (const orderByUnit of orderBy) if ((0, __entity_ts.is)(orderByUnit, require_mysql_core_columns_common.MySqlColumn)) orderByValues.push(__sql_sql_ts.sql.identifier(this.casing.getColumnCasing(orderByUnit)));
			else if ((0, __entity_ts.is)(orderByUnit, __sql_sql_ts.SQL)) {
				for (let i = 0; i < orderByUnit.queryChunks.length; i++) {
					const chunk = orderByUnit.queryChunks[i];
					if ((0, __entity_ts.is)(chunk, require_mysql_core_columns_common.MySqlColumn)) orderByUnit.queryChunks[i] = __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(chunk));
				}
	...
	_buildRelationalQuery({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (nestedQueryRelation) {
			let field = __sql_sql_ts.sql`json_array(${__sql_sql_ts.sql.join(selection.map(({ field, tsKey, isJson }) => isJson ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier(`${tableAlias}_${tsKey}`)}.${__sql_sql_ts.sql.identifier("data")}` : (0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) ? field.sql : field), __sql_sql_ts.sql`, `)})`;
			if ((0, __entity_ts.is)(nestedQueryRelation, ___relations_ts.Many)) field = __sql_sql_ts.sql`coalesce(json_arrayagg(${field}), json_array())`;
			...
			result = this.buildSelectQuery({
				table: (0, __entity_ts.is)(result, require_mysql_core_table.MySqlTable) ? result : new __subquery_ts.Subquery(result, {}, tableAlias),
				fields: {},
	...
	_buildRelationalQueryWithoutLateralSubqueries({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (config === true) selection = Object.entries(tableConfig.columns).map(([key, value]) => ({
			...
			for (const { tsKey: selectedRelationTsKey, queryConfig: selectedRelationConfigValue, relation } of selectedRelations) {
				...
				let fieldSql = __sql_sql_ts.sql`(${builtRelation.sql})`;
				if ((0, __entity_ts.is)(relation, ___relations_ts.Many)) fieldSql = __sql_sql_ts.sql`coalesce(${fieldSql}, json_array())`;
		...
		where = (0, __sql_expressions_index_ts.and)(joinOn, where);
		if (nestedQueryRelation) {
			let field = __sql_sql_ts.sql`json_array(${__sql_sql_ts.sql.join(selection.map(({ field }) => (0, __entity_ts.is)(field, require_mysql_core_columns_common.MySqlColumn) ? __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(field)) : (0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) ? field.sql : field), __sql_sql_ts.sql`, `)})`;
			if ((0, __entity_ts.is)(nestedQueryRelation, ___relations_ts.Many)) field = __sql_sql_ts.sql`json_arrayagg(${field})`;
			const nestedSelection = [{
				dbKey: "data",
				tsKey: "data",
				field,
