	...
	*/
	...
	_buildRelationalQuery({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (nestedQueryRelation) {
			let field = __sql_sql_ts.sql`json_array(${__sql_sql_ts.sql.join(selection.map(({ field }) => (0, __entity_ts.is)(field, __sqlite_core_columns_index_ts.SQLiteColumn) ? __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(field)) : (0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) ? field.sql : field), __sql_sql_ts.sql`, `)})`;
