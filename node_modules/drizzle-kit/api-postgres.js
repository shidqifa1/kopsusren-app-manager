...
const generateDrizzleJson = async (imports, prevId, schemaFilters, casing) => {
	...
	const { postgresSchemaError, postgresSchemaWarning } = await Promise.resolve().then(() => require("./views-DkfVL_-7.js")).then((n) => n.views_exports);
...
const startStudioServer = async (imports, credentials, options) => {
	...
	const { drizzleForPostgres, prepareServer } = await Promise.resolve().then(() => require("./studio-DK-jdELO.js"));
	const pgSchema = {};
	const relations = {};
