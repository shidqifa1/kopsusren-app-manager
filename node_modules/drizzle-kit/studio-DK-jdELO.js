...
const require_drizzle = require('./drizzle-Bj7v9CEh.js');
...
const getCustomDefaults = (schema, casing) => {
...
const drizzleForPostgres = async (credentials, pgSchema, relations, schemaFiles, casing) => {
	const { preparePostgresDB } = await Promise.resolve().then(() => require("./connections-BLAfvpUk.js"));
...
const drizzleForMySQL = async (credentials, mysqlSchema, relations, schemaFiles, casing) => {
	const { connectToMySQL } = await Promise.resolve().then(() => require("./connections-BLAfvpUk.js"));
...
const drizzleForSQLite = async (credentials, sqliteSchema, relations, schemaFiles, casing) => {
	...
	if ("driver" in credentials && credentials.driver === "d1") {
		const { connectToD1 } = await Promise.resolve().then(() => require("./connections-BLAfvpUk.js"));
	...
	const { connectToSQLite } = await Promise.resolve().then(() => require("./connections-BLAfvpUk.js"));
...
const extractRelations = (tablesConfig, casing) => {
	return Object.values(tablesConfig.tables).map((it) => Object.entries(it.relations).map(([name, relation]) => {
		try {
			...
			const fields = normalized.fields.map((it) => require_drizzle.getColumnCasing(it, casing)).flat();
			const refColumns = normalized.references.map((it) => require_drizzle.getColumnCasing(it, casing)).flat();
			let refSchema;
